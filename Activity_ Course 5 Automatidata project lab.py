...
# Outliers and really high or low numbers can mess up our predictions when we're trying to figure out how one thing affects another. So, after looking at the data, we'll come up with a plan to deal with these outliers, like getting rid of them or using average numbers instead.
# Sometimes, we don't have all the data we need. In those cases, we might have to guess what the missing data should be. We can do this by using the average or middle number of the data we do have.
# We also need to check if the things we're looking at in the data are related to each other. This helps us understand if our predictions are accurate or not.
# Sometimes, we can make new data out of the data we already have. For example, we can figure out how long something took by subtracting the time it ended from the time it started.
...
# Multiple linear regression helps us figure out one thing based on many other things.
# Exploratory data analysis helps us pick which things to use in our multiple linear regression, like numbers and categories.
# Sometimes, we have to try different combinations of things in our regression to find the best one that works well and follows the rules.

# **What results can be presented from this notebook?**
# We can talk about making sure our regression follows certain rules, and we can show how well our model predicts by using scores called MAE and RMSE.
